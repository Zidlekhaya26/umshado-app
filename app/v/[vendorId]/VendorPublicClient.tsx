          ...
          <div className="absolute bottom-0 left-0 right-0 p-6" style={{ background: 'linear-gradient(to top, rgba(0,0,0,0.8) 0%, transparent 100%)' }}>
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <h1 className="text-2xl font-bold text-white mb-1">{vendor.business_name}</h1>
                <p className="text-sm text-white/90 mb-2">{catIcon} {vendor.category}</p>
              ...
              {waHref && (
                <section className="rounded-2xl p-5" style={{ background: '#fff', border: '1.5px solid var(--border,#eadfd6)' }}>
                  ...
                  <div className="space-y-2">
                    <Link href={`/messages/new?vendorId=${vendorId}`} className="flex items-center justify-center gap-2 w-full px-4 py-3 rounded-xl font-semibold text-sm text-white active:scale-95 transition shadow-md" style={{ background: 'var(--primary,#6b1f3a)' }}>
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                      </svg>
                      Start Chat
                    </Link>
                    <a href={waHref} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 w-full px-4 py-3 rounded-xl border-2 font-semibold text-sm active:scale-95 transition" style={{ borderColor: 'var(--border,#eadfd6)', color: 'var(--primary,#6b1f3a)' }}>
                  ...
                  {packages.map(pkg => (
                    <div key={pkg.id} className="rounded-2xl p-5" style={{ background: '#fff', border: '1.5px solid var(--border,#eadfd6)' }}>
                      ...
                      <Link
                        ...
                        className="block w-full px-4 py-3 rounded-xl font-semibold text-sm text-center text-white active:scale-95 transition shadow-md"
                        style={{ background: 'var(--primary,#6b1f3a)' }}
                      >
                        Request Quote
                      </Link>
                    </div>
